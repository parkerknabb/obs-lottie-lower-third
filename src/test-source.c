...
static bool layer_matches_text_id(cJSON *layer,
                                  const char *wanted_nm,
                                  const char *wanted_ln)
{
                                         ...
                                         struct test_source *ctx)
{
    ...
    if (layer_matches_text_id(layer, "#text1", "text1")) {
        if (patch_text_document_value(layer, ctx->text1_value)) {
            ...
            blog(LOG_WARNING,
                 "[test_source] Found #text1 layer but could not patch text document");
        }
    }
    ...
    if (layer_matches_text_id(layer, "#text2", "text2")) {
        if (patch_text_document_value(layer, ctx->text2_value)) {
            ...
            blog(LOG_WARNING,
                 "[test_source] Found #text2 layer but could not patch text document");
        }
    }
}
...
static bool load_lottie_with_current_text(struct test_source *ctx)
{
    ...
    blog(LOG_INFO, "[test_source] Loading patched Lottie: %s",
         ctx->lottie_path);
    ...
    if (!json) {
        blog(LOG_ERROR, "[test_source] Failed to read Lottie file: %s",
             ctx->lottie_path);
        return false;
    }
    ...
    if (!root) {
        blog(LOG_ERROR, "[test_source] Failed to parse Lottie JSON: %s",
             ctx->lottie_path);
        return false;
    }
    ...
    if (cJSON_IsNumber(fr) && fr->valuedouble > 0.0) {
            ...
            blog(LOG_WARNING,
                 "[test_source] Lottie JSON has no valid 'fr'; using 30fps fallback");
        }
    ...
    if (ctx->lottie_width <= 0.0f || ctx->lottie_height <= 0.0f) {
        blog(LOG_WARNING,
             "[test_source] Lottie reported invalid size %.2fx%.2f; using buffer size",
    ...
    if (ctx->total_frames <= 1.0f) {
            blog(LOG_ERROR,
                 "[test_source] Lottie has invalid total frame count: %.2f",
                 ctx->total_frames);
            unload_lottie(ctx);
            return false;
        }
    ...
    blog(LOG_INFO, "[test_source] Lottie natural size: %.0fx%.0f",
         ctx->lottie_width, ctx->lottie_height);

    blog(LOG_INFO, "[test_source] Lottie total frames: %.1f",
         ctx->total_frames);
    ...
    blog(LOG_INFO,
         "[test_source] Patched Lottie loaded. text1=%d text2=%d",
         ctx->has_text1,
         ctx->has_text2);
...
static uint32_t get_exit_duration_ms(struct test_source *ctx)
{
    ...
    if (ctx->lottie_loaded && ctx->total_frames > 0) {
        ...
        if (outro_frames > 0) {
            ...
            blog(LOG_INFO, "[test_source] Calculated outro duration: %u ms (%.1f frames)",
                 duration_ms, outro_frames);
            return duration_ms;
        }
    }
...
static void test_source_begin_exit(struct test_source *ctx)
{
    if (ctx->state == STATE_HOLDING || ctx->state == STATE_ENTERING) {
        blog(LOG_INFO,
             "[test_source] begin_exit: EXITING from current Lottie frame %.2f",
...
static bool ensure_render_texture(struct test_source *ctx)
{
    ...
    if (!ctx->texture) {
        blog(LOG_ERROR,
             "[test_source] failed to create render texture %ux%u",
             w, h);
        return false;
    }
...
static bool render_buffer_to_obs(struct test_source *ctx,
                                 uint32_t draw_width,
...
static float get_lottie_preview_frame(struct test_source *ctx)
{
     ...
     */
    if (ctx->pvw_time_frame >= 0.0f &&
        ctx->pvw_time_frame < ctx->total_frames) {
     ...
     */
    if (ctx->hold_start_frame_lottie >= 0.0f &&
        ctx->hold_start_frame_lottie < ctx->total_frames) {
...
static obs_properties_t *test_get_properties(void *data)
{
    ...
    obs_properties_add_path(props, "lottie_path", "Lottie File", OBS_PATH_FILE,
...
static void test_show(void *data)
{
    ...
    blog(LOG_INFO, "[test_source] Starting at intro frame: %.1f",
         ctx->intro_start_frame);
}
...
static void *test_create(obs_data_t *settings, obs_source_t *source)
{
    ...
    tvg_swcanvas_set_target(ctx->canvas, ctx->buffer,
                            ctx->buffer_width, ctx->buffer_width, ctx->buffer_height,
...
static void test_destroy(void *data)
{
    ...
    if (ctx->lottie_path) bfree(ctx->lottie_path);
    if (ctx->canvas) tvg_canvas_destroy(ctx->canvas);
    if (ctx->anim) tvg_animation_del(ctx->anim);
    destroy_render_texture(ctx);
    if (ctx->buffer) bfree(ctx->buffer);
    if (ctx->text1_value) bfree(ctx->text1_value);
    if (ctx->text2_value) bfree(ctx->text2_value);
...
static void test_render(void *data, gs_effect_t *effect)
{
    ...
    if (render_log_counter % 60 == 0) {
        blog(LOG_INFO,
             "[test_source] render: state=%d loaded=%d current=%.2f preview=%.2f total=%.2f path=%s",
             ctx->state,
             ctx->lottie_loaded,
             ctx->current_lottie_frame,
             get_lottie_preview_frame(ctx),
             ctx->total_frames,
             ctx->lottie_path ? ctx->lottie_path : "(null)");
    }
    ...
    render_buffer_to_obs(ctx,
                         (uint32_t)ctx->buffer_width,
...
static void test_source_tick_exit(struct test_source *ctx,
    ...
    if (ctx->state == STATE_EXITING) {
        if (ctx->lottie_loaded) {
             ...
             */
            if (ctx->hold_end_frame_lottie >= 0.0f &&
                ctx->outro_start_frame >= 0.0f &&
            ...
            if (next_frame >= ctx->total_frames - 1.0f) {
                ...
                blog(LOG_INFO,
                     "[test_source] tick_exit: animation complete at frame %.2f",
                     ctx->current_lottie_frame);
            } else {
        ...
        if (ctx->current_frame % 10 == 0) {
            blog(LOG_INFO,
                 "[test_source] tick_exit: transition_frame=%u lottie=%.2f/%.1f",
                 ctx->current_frame,
                 ctx->current_lottie_frame,
                 ctx->total_frames);
        }
