...
static void *test_create(obs_data_t *settings, obs_source_t *source)
{
    ...
    if (tvg_picture_load(ctx->pic, LOTTIE_TEST_PATH) != TVG_RESULT_SUCCESS) {
        ...
        tvg_animation_get_total_frame(ctx->anim, &total_frames);
        tvg_animation_get_duration(ctx->anim, &duration);
        ctx->lottie_fps = (duration > 0) ? total_frames / duration : 30.0f;
        blog(LOG_INFO, "[test_source] Lottie: %.0f frames @ %.2f fps",
             total_frames, ctx->lottie_fps);
        ctx->comp_w = (uint32_t)pw;
        ctx->comp_h = (uint32_t)ph;
        tvg_picture_set_size(ctx->pic, ctx->comp_w, ctx->comp_h);   // native, no distortion
...
static void test_video_tick(void *data, float seconds)
{
    ...
    // Advance Lottie frame counter based on real elapsed time
    // lottie_fps should be stored in the struct — set it during load
    if (ctx->lottie_loaded && ctx->state != STATE_HIDDEN) {
        ctx->lottie_frame += seconds * ctx->lottie_fps;
    }
    
    if(ctx->lottie_frame > ctx->end_frame) {
        ctx->lottie_frame = ctx->end_frame;
    }
...
static void test_render(void *data, gs_effect_t *effect)
{
    ...
    // Render Lottie frame (off-GPU)
    Tvg_Canvas canvas = tvg_swcanvas_create(TVG_ENGINE_OPTION_DEFAULT);
    tvg_swcanvas_set_target(canvas, ctx->buf, ctx->comp_w, ctx->comp_w, ctx->comp_h, TVG_COLORSPACE_ARGB8888);
    tvg_canvas_add(canvas, ctx->pic);
    tvg_animation_set_frame(ctx->anim, ctx->lottie_frame);
    ...
    // Upload to GPU texture (ON GRAPHICS THREAD)
    obs_enter_graphics();
    if (!ctx->texture) {
        ctx->texture = gs_texture_create(ctx->comp_w, ctx->comp_h, GS_BGRA, 1, NULL, GS_DYNAMIC);
    }
    if (ctx->texture) {
        gs_texture_set_image(ctx->texture, (const uint8_t *)ctx->buf, ctx->comp_w * 4, false);
    }
    obs_leave_graphics();
    ...
    while (gs_effect_loop(def, "Draw"))
        gs_draw_sprite(ctx->texture, GS_FLIP_V, ctx->comp_w, ctx->comp_h);
}
...
{
    //struct test_source *ctx = (struct test_source *)data;
    //return ctx->lottie_loaded ? ctx->comp_h : 1080;
    UNUSED_PARAMETER(data);
    return 1080;
}
